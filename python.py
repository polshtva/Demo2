...
def open_windowTask(user_id):
    ...
    btn_save = Button(new_windowTask, text='Отправить заявку', command=lambda: save_and_update_treeview(current_date, equipment_entry, type_server_entry, desc_server_entry, user_id, status_var,new_windowTask))
    btn_save.pack()
    
    btn_close = Button(new_windowTask, text='Закрыть', command=lambda: closeWindow(new_windowTask))
    btn_close.pack()

def save_and_update_treeview(current_date, equipment_entry, type_server_entry, desc_server_entry, user_id, status,new_windowTask):
    writeDataTask(current_date, equipment_entry.get(), type_server_entry.get(), desc_server_entry.get("1.0", "end-1c"), user_id, status)
    closeWindow(new_windowTask)

def writeDataTask(data, equipment, type, desc, user_id, status):
    ...
    dbCursor.execute("INSERT INTO Servers (Data_Server, Equipment, Type_Server, Desc_Server, Id_Client, Status_Server) VALUES (?, ?, ?, ?, ?, ?)", (data, equipment, type, desc, user_id, status))
...
def open_userwindow(user_id):
    ...
    tree = ttk.Treeview(new_windowUser, columns=("Id", "Date", "Equipment", "Type", "Description", "Client", "Status"), show="headings")
    ...
    tree.column("Client", anchor="center", width=360)
    tree.column("Status", anchor="center", width=360)
...
def open_employwindow(user_id):
    new_windowEmploy = Tk()
    new_windowEmploy.title("Окно юзера")
